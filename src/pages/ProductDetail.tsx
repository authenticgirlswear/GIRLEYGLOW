...
export const ProductDetailPage: React.FC = () => {
  ...
  const productSchema = {
    ...
    url: `https://authentic-girlswear.vercel.app/product/${product.slug}`,
    image: product.images.filter((img: string) => img.startsWith('http')),
    brand: {
      '@type': 'Brand',
      name: 'Authentic Girlswear',
    },
  ...
  return (
    <>
      {/* ── SEO: Helmet with OpenGraph + JSON-LD Product Schema ── */}
      <Helmet>
        <title>{product.name} | Authentic Girlswear</title>
        <meta name="description" content={product.shortDescription || product.description?.slice(0, 155) || `Buy ${product.name} at Authentic Girlswear. Best price in Bangladesh.`} />
        <link rel="canonical" href={`https://authentic-girlswear.vercel.app/product/${product.slug}`} />
        <meta property="og:title" content={product.name} />
        <meta property="og:description" content={product.shortDescription || product.description?.slice(0, 155) || ''} />
        <meta property="og:url" content={`https://authentic-girlswear.vercel.app/product/${product.slug}`} />
