/* ===================================================
   AUTHENTIC GIRLSWEAR - Product Detail Page (ENHANCED)
   ...
   ✅ ALL routing
   ✅ ALL GTM events
   ✅ ALL Facebook Pixel events
   ✅ ALL color resolution logic
   ...
   ✅ Review UI support & rating display
   ✅ Stock urgency display
   ✅ Cloudinary optimized images with lazy loading
   ✅ Better image alt text with variants
...
// ✅ ADDED: Import centralized config files
...
/**
 * ✅ ADDED: Cloudinary image optimization helper
...
const getOptimizedImageUrl = (
  ...
  // Add optimization transformations: width, quality, auto format, auto DPR
  const transformations = `w_${width},q_${quality},f_auto,dpr_auto`;
  return `${parts[0]}/upload/${transformations}/${parts[1]}`;
};

/**
 * ✅ ADDED: Generate better image alt text with product info
 */
const getImageAlt = (
  productName: string,
  ...
  return `${productName}${colorText}${sizeText} - Image ${index + 1}`;
};

/**
 * ✅ ADDED: Calculate stock urgency status
 */
const getStockInfo = (stock: number) => {
  if (stock === 0) {
    return { status: 'out-of-stock', message: 'Out of Stock', urgent: false, color: 'red' };
  } else if (stock <= 5) {
    return {
      status: 'low-stock',
    ...
    return {
      ...
      message: `Limited stock: ${stock} available`,
      urgent: true,
      color: 'orange'
    };
  }
...
export const ProductDetailPage: React.FC = () => {
  ...
  /**
   * ✅ ADDED: Generate Product JSON-LD Schema for SEO
   */
  const generateProductSchema = () => {
    ...
    const schema = {
      ...
      image: product.images.map((img) => getOptimizedImageUrl(img, 1200)),
  ...
  /**
   * ✅ ADDED: Handle sticky cart visibility on scroll
   */
  useEffect(() => {
    const handleScroll = () => {
  ...
  const finalPrice = product.comparePrice || currentPrice;
  ...
  return (
    <>
      {/* ✅ ADDED: Dynamic SEO Meta Tags with Helmet */}
      <Helmet>
        <title>{`${product.name} | ${siteConfig.websiteName}`}</title>
        <meta
          name="description"
          content={
            product.description ||
            product.shortDescription ||
            `Buy ${product.name} at ${siteConfig.websiteName}. ${product.comparePrice
              ? `Special price: ৳${product.price}`
              : `Price: ৳${product.price}`
            }. ${stockInfo.message}. ${product.reviewCount > 0 ? `${product.reviewCount} customer reviews.` : ''
            }`
          }
        />
        <meta
          name="keywords"
          content={`${product.name}, ${product.category}, ${siteConfig.websiteName}, buy ${product.name}, ${product.tags?.join(', ')}`}
        />
        <link
          rel="canonical"
          href={`${siteConfig.domain}/product/${product.slug}`}
        />

        {/* Open Graph */}
        <meta property="og:type" content="product" />
        <meta property="og:title" content={`${product.name} | ${siteConfig.websiteName}`} />
        <meta
          property="og:description"
          content={product.description || product.shortDescription || product.name}
        />
        <meta
          property="og:image"
          content={getOptimizedImageUrl(product.images[0], 1200)}
        />
        <meta
          property="og:url"
          content={`${siteConfig.domain}/product/${product.slug}`}
        />
        ...
        <meta name="twitter:title" content={`${product.name} | ${siteConfig.websiteName}`} />
        <meta
      ...
      <div className="min-h-screen pt-24 pb-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-10">
                        ...
                        >
                          {img.startsWith('http') ? (
                            <img
                              ...
                              alt={getImageAlt(product.name, selectedColor, selectedSize, i)}
                              loading="lazy"
            ...
            <FadeIn delay={0.2}>
              <div>
                ...
                {/* ✅ ADDED: Stock Urgency Display */}
                {stockInfo.urgent && stockInfo.status !== 'out-of-stock' && (
                  <div className={`flex items-center gap-2 p-3 rounded-lg mb-4 ${stockInfo.color === 'orange'
                      ? 'bg-orange-50 border border-orange-200'
                      : 'bg-red-50 border border-red-200'
                    }`}>
                    <AlertCircle className={`w-5 h-5 ${stockInfo.color === 'orange' ? 'text-orange-600' : 'text-red-600'
                      }`} />
        ...
        {/* ✅ ADDED: Sticky Mobile Add to Cart */}
        <AnimatePresence>
          {showStickyCart && (
            <motion.div
              initial={{ y: 100 }}
